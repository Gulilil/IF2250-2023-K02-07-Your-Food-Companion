...
class ExpiredFoodTab(QWidget):
    currentCategory = "All Categories"
    def __init__ (self):
        super(ExpiredFoodTab, self).__init__()
        ...
        # # Table data
        self.header = im.Expired_Food_Data_Header
        self.data = im.getExpiredFoodData(None)
    ...
    def changeCategory(self, category):
        ...
        if category == "All Categories":
          ...
          self.data = im.getExpiredFoodData(category)
    ...
    def changeSearch(self):
        ...
        if (category == "All Categories"):
            temp = im.getExpiredFoodData(None)
        else:
            temp = im.getExpiredFoodData(category)
        self.data = []
        for i in temp:
            if (content in i[0]):
                self.data.append(i)
